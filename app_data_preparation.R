...
rap_adm <- readRDS(paste0(data_folder, "rapid/Admissions_by_category_14-Sep.rds")) %>% 
  janitor::clean_names() %>% 
...
prepare_final_data(rap_adm, "rapid", last_week = "2020-09-06", 
...
ooh_may_onwards <- read_excel(paste0(data_folder, "GP_OOH/WIDER IMPACT PC OOH Data_56_115720611879574904.xlsx")) %>% 
  janitor::clean_names() %>%
...
prepare_final_data(dataset = ooh, filename = "ooh", last_week = "2020-08-30")
...
prepare_final_data(ae_data, "ae", last_week = "2020-09-06")
...
nhs24 <-  rbind(readRDS(paste0(data_folder, "NHS24/NHS24 01Jan2018 to 07Jun2020.rds")),
                read_tsv(paste0(data_folder, "NHS24/NHS24_report_08062020to13092020.txt"))) %>%
  janitor::clean_names() %>% 
...
# Formatting file for shiny app
prepare_final_data(dataset = nhs24, filename = "nhs24", last_week = "2020-09-13")
...
# Formatting file for shiny app
prepare_final_data(dataset = sas, filename = "sas", last_week = "2020-08-30")
...
prepare_final_data(ae_cardio, "ae_cardio", last_week = "2020-09-06")

###############################################.
## Prescribing - Cardiovascular Drugs ----
###############################################.
cardio_drugs <- read_xlsx(paste0(data_folder, "prescribing_cardio/covid emessage AMS only 20200910.xlsx")) %>% 
  select(1:5) %>% 
  clean_names() %>% 
...
prepare_final_data(cardio_drugs, "cardio_drugs", last_week = "2020-09-06")
