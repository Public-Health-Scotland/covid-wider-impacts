...
dataset <- dataset %>%
  ...
  # If isoweek 53, change to isoweek 52 so that we can compare to 2018/19
  # (53 isoweeks in 2020 but only 52 in 2018 & 2019)
  mutate(week_no = if_else(week_no == 53, 52, week_no))
...
data_2020 <- left_join(dataset %>%
                         filter(year(week_ending) %in% c("2020", "2021")),
                       historic_data) %>%
  ...
  mutate(variation = case_when(count_average == 0 & variation == 0 &
                                 count != 0 ~ 100,
                               TRUE ~ variation)) %>%
  # Remove week no variable
  select(-week_no)

# Store data in weekly data
weekly_data <- data_2020

# Add in weekly marker
...
# Creating month number to be able to compare pre-covid to covid period ----
dataset <- dataset %>%
  mutate(month_no = month(week_ending),
         # Fixing area names
         area_name = gsub(" and ", " & ", area_name))
## apply disclosure flag to cases ----
dataset <- disc_flag_all()
...
# Creating average appts of pre-covid data (2018-2019) by week of the year ----
historic_data <- dataset %>% filter(year(week_ending) %in% c("2018", "2019")) %>%
  group_by(category, type, area_name, area_type, month_no, appt_type, spec) %>%
  summarise(count_average = round((sum(count, na.rm = T))/2, 1))
...
# Store data in monthly data
monthly_data <- data_2020

# Add in a marker for monthly data
monthly_data <- monthly_data %>%
  mutate(time_split = "Monthly")

# Combine data
op_data <- bind_rows(weekly_data, monthly_data)

# If we only want weekly data for now, remove monthly data
# op_data <- op_data %>%
#   filter(time_split = "Weekly") %>%
#   select(-time_split)

# Save out data
saveRDS(op_data, paste0(data_folder, "outpats.rds"))
saveRDS(op_data, paste0(WID_folder,"final_app_files/outpats_",
                        format(Sys.Date(), format = '%d_%b_%y'), ".rds"))
