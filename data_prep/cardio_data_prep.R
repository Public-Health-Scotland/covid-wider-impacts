...
ae_cardio <- read_xlsx(paste0(ae_folder, "2021-05-27-CardioVascular-AttendancesDuringCovid-19.xlsx")) %>% 
  clean_names() %>% 
...
prepare_final_data(ae_cardio, "ae_cardio", last_week = "2021-05-23")

###############################################.
## OOH Cardiac  ----
###############################################.

ooh_data_cardiac <- read_csv(paste0(data_folder, "GP_OOH_Cardio/2021-05-31-Weekly Cardio Diagnosis OOH extract.csv")) %>% 
  janitor::clean_names() %>% 
...
prepare_final_data_cardiac(dataset = ooh_cardiac, filename = "ooh_cardiac", last_week = "2021-05-23")

###############################################.
## SAS Cardiac ----
###############################################.

sas_data_cardiac <- read_csv(paste0(data_folder,"SAS_Cardio/2021-05-31-Weekly Cardio Diagnosis SAS extract.csv")) %>%
  janitor::clean_names() %>% 
  filter(age > 14) %>% # Filter age > 14
...
# Formatting file for shiny app
prepare_final_data_cardiac(dataset = sas_cardiac, filename = "sas_cardiac", last_week = "2021-05-23")

###############################################.
## Prescribing - Cardiovascular Drugs ----
###############################################.
cardio_drugs <- read_xlsx(paste0(data_folder, "prescribing_cardio/2021-05-27-covid emessage AMS only.xlsx")) %>% 
  select(1:5) %>% 
  clean_names() %>% 
...
prepare_final_data(cardio_drugs, "cardio_drugs", last_week = "2021-05-23")
