...
create_ui <- function(last_month) {
  ...
  query_smr01 <- glue("SELECT LINK_NO, DR_POSTCODE pc7, SEX, ADMISSION_DATE, ADMISSION_TYPE, 
                        ...
                        WHERE discharge_date between '1 January 2018' and '{sql_date}'
  ...
  # Creat flags for the different types of unintentional injury 
  tab1 %<>% 
  ...
  #Create locations of where the injury happen
  tab1 %<>% 
  ...
  # if more than 1 UI is recorded per episode, the location is taken from the injury closer to the primary position
    mutate(diagwithloc = case_when(diagwithloc1 !="0" ~ diagwithloc1,
                                   diagwithloc2 !="0" ~ diagwithloc2,
                                   diagwithloc3 !="0" ~ diagwithloc3,
                                   diagwithloc4 !="0" ~ diagwithloc4,
                                   diagwithloc5 !="0" ~ diagwithloc5,
                                   diagwithloc6 !="0" ~ diagwithloc6, T ~ "0")) %>% 
  # if RTA but also poison, falls, struck, crushing, scalds and accepx, fix location to undisclosed instead of NA
    mutate(injurylocation = case_when(admission_type == "32" ~ "NA",
                                     diagwithloc =="NA" & (poison ==1 | falls ==1 | struck ==1 | crushing ==1 | scalds ==1 | accexp ==1) ~ "Undisclosed", 
                                     TRUE ~ diagwithloc)) %>% 
